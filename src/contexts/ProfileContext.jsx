    ...
    try {
      ...
      if (savedProfiles) {
        ...
        if (savedActiveProfileId) {
          ...
          if (activeProfile) {
            dispatch({
              type: PROFILE_ACTIONS.SET_ACTIVE_PROFILE,
  ...
  const createDefaultProfiles = useCallback(async userId => {
    try {
      ...
      dispatch({
        ...
        payload: defaultProfiles,
      });
      dispatch({
        type: PROFILE_ACTIONS.SET_ACTIVE_PROFILE,
        payload: defaultProfiles[0],
      });
    ...
    async (profileId, updates) => {
      try {
        ...
        if (state.activeProfile?.id === profileId) {
          const updatedActiveProfile = updatedProfiles.find(
            p => p.id === profileId,
          );
          dispatch({
            type: PROFILE_ACTIONS.SET_ACTIVE_PROFILE,
            payload: updatedActiveProfile,
          });
        }
